(function() {
  ...
  // Кеш для результатів пошуку (покращення #2)
  var search_cache = {};
  var cache_timeout = 1000 * 60 * 30; // 30 хвилин

  // Автовизначення якості на основі швидкості з'єднання (покращення #3)
  function getOptimalQuality() {
    ...
    // Якщо є Network Information API
    if (connection) {
  ...
  function fxapi(component, _object) {
    ...
    // Покращена авторизація з продовженою сесією (покращення #1)
    if (!fxapi_token) {
      var user_code = '';
      var user_token = '';
      ...
      // Перевірка чи не застаріла сесія (30 днів)
      ...
      ping_auth = setInterval(function() {
        network.silent(Lampa.Utils.addUrlComponent(api_url + 'user_profile', dev_token + user_token), function(json) {
          if (json && json.user_data) {
            ...
            Lampa.Storage.set("fxapi_auth_time", Date.now()); // Зберігаємо час авторизації
    ...
    // Покращений пошук з кешуванням (покращення #2)
    this.searchByTitle = function(_object, query) {
      ...
      network.clear();
      network.timeout(15000); // Збільшено таймаут
    ...
    function extractData(data) {
      ...
      var optimal_quality = getOptimalQuality(); // Автовизначення якості
          ...
          for (var voice in episode) {
            ...
            for (var ID in episode_voice) {
              ...
              var quality_eps = file_episod.qualities.filter(function(qualitys) {
                return qualitys <= optimal_quality; // Використовуємо оптимальну якість
              });
    ...
    // Покращений інтерфейс фільтрів (покращення #4)
    function filter() {
      filter_items = {
        season: [],
        voice: [],
      ...
      // Сортування озвучок для кращого відображення
      ...
      voice_with_info.sort(function(a, b) {
        // Пріоритет для українських озвучок
              ...
              episode.folder.forEach(function(media) {
                if (media.translation == filter_items.voice_info[choice.voice].id) {
                  filtred.push({
                    ...
                    info: '🎬 ' + filter_items.voice[choice.voice] // Додаємо іконку
                  });
                }
              });
            }
  ...
  function component(object) {
    ...
    this.filter = function(filter_items, choice) {
      ...
      if (filter_items.voice && filter_items.voice.length) add('voice', 'Озвучка');
  ...
  function startPlugin() {
    ...
    // Змінюємо назву на "Filmix" замість "Дивитись онлайн"
    var manifest = {
      type: 'video',
      version: '1.2.0',
      name: 'Filmix',
